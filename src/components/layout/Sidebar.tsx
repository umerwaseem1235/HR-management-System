...
export default function Sidebar({ mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const saveNavScroll = () => {
    try {
      if (navRef.current) sessionStorage.setItem('hrms_sidebar_scroll', String(navRef.current.scrollTop));
    } catch {
  ...
  const renderSidebarContent = (compact: boolean) => (
    <div className={`relative flex h-full flex-col overflow-hidden bg-gradient-to-b from-[#1a3a5c] via-[#17324D] to-[#0d1f33] text-white shadow-2xl shadow-[#0d1f33]/40 sidebar-panel ${compact ? 'w-[76px]' : 'w-[264px]'}`}>
      {/* Decorative glows */}
      ...
      {/* Logo */}
      <div className={`flex h-16 flex-shrink-0 items-center px-4 ${compact ? 'justify-center' : ''}`}>
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className={`min-w-0 overflow-hidden whitespace-nowrap sidebar-fade ${compact ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <nav ref={navRef} className="sidebar-scroll flex-1 space-y-1 overflow-y-auto overflow-x-hidden px-3 py-4">
        {filteredNav.map(item => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              onClick={() => { saveNavScroll(); onMobileClose(); }}
            ...
            >
              ...
              <span className={`truncate whitespace-nowrap sidebar-fade ${compact ? 'opacity-0' : 'opacity-100'}`}>{item.name}</span>
              {item.badge && item.badge > 0 && (
                <span className={`ml-auto flex h-5 min-w-5 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-red-500 to-red-600 px-1.5 text-[11px] font-bold text-white shadow-sm ring-1 ring-white/30 sidebar-fade ${compact ? 'opacity-0' : 'opacity-100'}`}>
                  {item.badge}
                </span>
              )}
            </Link>
      ...
      <div className="flex-shrink-0 p-3">
        <div className="rounded-2xl border border-white/15 bg-white/10 p-3 shadow-inner">
          <div className="flex items-center gap-3">
            ...
            <div className={`flex min-w-0 flex-1 items-center gap-2 overflow-hidden whitespace-nowrap sidebar-fade ${compact ? 'opacity-0' : 'opacity-100'}`}>
              ...
              <button
                onClick={logout}
                className="flex-shrink-0 rounded-lg p-1.5 text-white/50 transition-colors hover:bg-red-500/20 hover:text-red-300"
                title="Logout"
              >
                <LogOut size={16} />
              </button>
